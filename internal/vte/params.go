...
func (b *paramsBuf) param(c byte) {
	switch c {
	case ';':
		// Advance to next parameter; materialize first param if not yet seen.
		if b.n == 0 {
			b.n = 1
			b.nSub[0] = 0
		}
	...
	case ':':
		// Advance to next sub-parameter within the current param.
		if b.n == 0 {
			b.n = 1
			b.nSub[0] = 1
			b.flat[0] = 0
...
func (b *paramsBuf) build(ps [][]uint16, flat []uint16) ([][]uint16, []uint16) {
	...
	for i := 0; i < b.n; i++ {
		...
		n := b.nSub[i]
		if n == 0 {
			// Empty param → implicit default value 0.
			flat = append(flat, 0)
		} else {
			base := i * maxSubParams
			for j := 0; j < n; j++ {
