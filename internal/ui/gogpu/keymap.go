...
// encode converts a key press event to the byte sequence written to the PTY.
// Returns nil for keys that produce no output (modifier-only, etc.).
// Printable characters are handled via OnTextInput, not here.
func encode(key gpucontext.Key, mods gpucontext.Modifiers, appCursor bool) []byte {
	...
	// Application cursor mode swaps arrow escape sequences.
	if appCursor {
...
// ctrlByte maps a key + Ctrl to a C0 control byte.
func ctrlByte(key gpucontext.Key) (byte, bool) {
...
// Matches kitty's terminfo entries for maximum compatibility.
...
// appCursorSeqs replaces arrow sequences when DECCKM (?1h) is active.
